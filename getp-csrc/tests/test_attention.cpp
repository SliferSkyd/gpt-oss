...
static void launch_optimized(
  ...
  hipLaunchKernelGGL(
    ...
    batch_size, n_heads, n_kv_heads, head_dim,
    seq_len, n_layers, layer_idx, use_sw,
    batch_kv_stride, layer_kv_offset, 64
  );
}
// static void launch_optimized(
//     dim3 grid, dim3 block, size_t shmem /*ignored*/, hipStream_t stream,
//     float* out, const float* q,
//     const __hip_bfloat16* key_cache, const __hip_bfloat16* value_cache,
//     const __hip_bfloat16* sinks, const float* mask, const int* seq_lengths,
//     int batch_size, int n_heads, int n_kv_heads, int head_dim,
//     int seq_len, int n_layers, int layer_idx, bool use_sw,
//     size_t batch_kv_stride, size_t layer_kv_offset, int tile_t_req)
// {
//   const int tile_t = cap_tile_t_for_shmem_bf16(tile_t_req, head_dim, 64u*1024u);
//   const size_t width = (size_t)head_dim + 1;
//   const size_t shmem_needed = 2u * (size_t)tile_t * width * sizeof(__hip_bfloat16); // K + V (bf16)

//   hipLaunchKernelGGL(
//     fused_attention_kernel_optimized_bf16lds,
//     grid, block, shmem_needed, stream,
//     out, q, key_cache, value_cache,
//     sinks, mask, seq_lengths,
//     batch_size, n_heads, n_kv_heads, head_dim,
//     seq_len, n_layers, layer_idx, use_sw,
