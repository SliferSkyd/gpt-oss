...
uint8_t *d_w_mlp1_mxfp4, *d_w_mlp2_mxfp4;  // Packed MXFP4 indices
float *d_w_mlp1_scales, *d_w_mlp2_scales;   // MXFP4 block scales
__hip_bfloat16 *d_b_mlp1, *d_b_mlp2;        // Biases remain in bfloat16
...
// Kernel for MXFP4 matrix multiplication
__global__ void matmul_kernel_mxfp4(float *output, const float *input, 
                                     const uint8_t *weight_packed, const float *weight_scales,
                                     int batch_size, int input_dim, int output_dim,
                                     size_t total_weight_elements)
{
    ...
    for (int j = 0; j < input_dim; j++)
    {
        ...
        size_t weight_idx = (size_t)out_idx * input_dim + j;
        
        // Dequantize MXFP4 weight to FP32
        float weight_fp32 = dequantize_mxfp4(weight_packed, weight_scales, weight_idx, total_weight_elements);
...
void matmul_mxfp4(
    ...
    hipStream_t stream = nullptr)
{
    // Use simple kernel for MXFP4 initially
    dim3 grid(batch_size, (output_dim + 31) / 32);
    dim3 block(1, 32);
    
    hipLaunchKernelGGL(
        matmul_kernel_mxfp4,
        grid, block, 0, stream,
        output, input, weight_packed, weight_scales,
        batch_size, input_dim, output_dim, total_weight_elements);
}
...
void malloc_gpu_run_state(RunState *s, Config *p)
{
    ...
    HIP_CHECK(hipMalloc((void **)&d_w_mlp1_mxfp4, mlp1_packed_size));
    HIP_CHECK(hipMalloc((void **)&d_w_mlp1_scales, mlp1_num_blocks * sizeof(float)));
    ...
    HIP_CHECK(hipMalloc((void **)&d_w_mlp2_mxfp4, mlp2_packed_size));
    HIP_CHECK(hipMalloc((void **)&d_w_mlp2_scales, mlp2_num_blocks * sizeof(float)));
...
void copy_weights_to_gpu(Transformer *transformer)
{
    ...
    // Convert and copy MoE weights using MXFP4 quantization
    size_t mlp1_size = p->n_layers * p->n_experts * (2 * p->intermediate_dim) * p->hidden_dim;
    MXFP4Weights mlp1_mxfp4;
    quantize_to_mxfp4(w->w_mlp1, mlp1_mxfp4, mlp1_size);
    size_t mlp1_packed_size = (mlp1_size + 1) / 2;
    HIP_CHECK(hipMemcpy(d_w_mlp1_mxfp4, mlp1_mxfp4.packed_values, mlp1_packed_size, hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_w_mlp1_scales, mlp1_mxfp4.scales, mlp1_mxfp4.num_blocks * sizeof(float), hipMemcpyHostToDevice));
    ...
    HIP_CHECK(hipMemcpy(d_w_mlp2_mxfp4, mlp2_mxfp4.packed_values, mlp2_packed_size, hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_w_mlp2_scales, mlp2_mxfp4.scales, mlp2_mxfp4.num_blocks * sizeof(float), hipMemcpyHostToDevice));
...
void finish(Transformer *transformer, Tokenizer *tokenizer)
{
    ...
    // Free MXFP4 weights
    HIP_CHECK(hipFree(d_w_mlp1_mxfp4));
    HIP_CHECK(hipFree(d_w_mlp1_scales));
    HIP_CHECK(hipFree(d_b_mlp1));
    HIP_CHECK(hipFree(d_w_mlp2_mxfp4));
    HIP_CHECK(hipFree(d_w_mlp2_scales));
    ...
    for (int expert_id = 0; expert_id < n_experts; expert_id++)
    {
        ...
        // MLP1 (Gate/Up projections)
        {
            ...
            matmul_mxfp4(mlp1_out_ptr, expert_input_ptr,
                        d_w_mlp1_mxfp4 + expert_packed_offset,
                        d_w_mlp1_scales + expert_scale_offset,
        ...
        // MLP2 (Down projection)
        {
            ...
            matmul_mxfp4(expert_output_ptr, gate_up_ptr,
                        d_w_mlp2_mxfp4 + expert_packed_offset_mlp2,
                        d_w_mlp2_scales + expert_scale_offset_mlp2,
