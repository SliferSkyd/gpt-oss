...
struct Timer
{
        ...
        : name(timer_name), use_hip(is_hip), write_immediately(immediate_write)
    {
        ...
        // No HIP event creation needed when using device-wide sync timing
    }

    ~Timer()
    {
        // No HIP event objects to destroy
    }

    void start()
    {
        if (use_hip) {
            // Ensure previous GPU work is finished so we only time the region of interest
            HIP_CHECK(hipDeviceSynchronize());
        }
    ...
    {
        if (use_hip) {
            // Wait for all GPU work (any streams) launched since start() to complete
            HIP_CHECK(hipDeviceSynchronize());
        }
