                                ...
                                const TPGroup& tp, hipStream_t s)
{
  ...
  // bounce buffer (only allocated if needed)
  ...
  for (int step = 0; step < TP-1; ++step) {
    ...
    for (int t = 0; t < tiles; ++t) {
      ...
      if (tp.p2p[my][prev]) {
        ...
        if (!h_stage) HIP_CHECK(hipHostMalloc(&h_stage, TILE_ROWS*cols_bytes));
        // **CHANGED:** bounce with synchronous copies (safe across devices)
        HIP_CHECK(hipMemcpy(h_stage,   src_peer,  bytes, hipMemcpyDeviceToHost));
        HIP_CHECK(hipMemcpy(dst_local, h_stage,   bytes, hipMemcpyHostToDevice));
      }
    }
  ...
  if (h_stage) HIP_CHECK(hipHostFree(h_stage));
}
                               ...
                               hipStream_t      s)
{
  ...
  const size_t TILE_ELEMS = std::max<size_t>(1, (size_t)RING_TILE_BYTES / sizeof(float));

  // Device recv tile
  float* d_recv = nullptr;
  HIP_CHECK(hipMalloc((void**)&d_recv, TILE_ELEMS * sizeof(float)));

  // Optional host bounce tile (allocate only if needed)
  void* h_stage = nullptr;
  const bool need_bounce = !tp.p2p[my][prev];
  if (need_bounce) HIP_CHECK(hipHostMalloc(&h_stage, TILE_ELEMS * sizeof(float)));
  ...
  for (int step = 0; step < TP - 1; ++step) {
    ...
    while (done < rc.nelems) {
      ...
      if (!need_bounce) {
        // P2P: pull tile from prev into my d_recv
        HIP_CHECK(hipMemcpyPeerAsync(d_recv, my_dev,
                                     peer_bases[prev] + off, tp.group_devs[prev],
                                     bytes, s));
      } else {
        // Host bounce
        HIP_CHECK(hipMemcpyAsync(h_stage, peer_bases[prev] + off, bytes,
                                 hipMemcpyDeviceToHost, s));
        HIP_CHECK(hipMemcpyAsync(d_recv, h_stage, bytes,
  ...
  for (int step = 0; step < TP - 1; ++step) {
    ...
    while (done < rc.nelems) {
      ...
      if (!need_bounce) {
        ...
        HIP_CHECK(hipMemcpyAsync(h_stage, peer_bases[prev] + off, bytes,
                                 hipMemcpyDeviceToHost, s));
        HIP_CHECK(hipMemcpyAsync(buf_local + off, h_stage, bytes,
                                 hipMemcpyHostToDevice, s));
      }
      done += take;
    }
