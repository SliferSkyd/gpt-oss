...
#ifndef FAST_EXP_APPROX
#define FAST_EXP_APPROX 1
#endif

// Use C math functions (AMD) or CUDA intrinsics (NVIDIA). Clamp to avoid overflow.
__device__ inline float exp_fast(float x) {
  ...
  constexpr float LOG2E = 1.4426950408889634f;
    ...
    // HIP/ROCm: use the standard device math function
    return ::exp2f(x * LOG2E);
  #elif defined(__CUDA_ARCH__)
    // CUDA: fast intrinsic is available
    return __exp2f(x * LOG2E);
  #else
    // Host or unknown: be conservative
    return ::expf(x);
  #endif
#else
  #if defined(__HIP_PLATFORM_AMD__)
    return ::expf(x);
  #else
    return __expf(x); // CUDA fast expf
  #endif
#endif
}
