...
__global__ void paged_attention_scores_kernel(float *att, const float *q, 
                                              const __hip_bfloat16 *paged_key_cache,
                                              const int *block_table, const int *positions,
                                              int batch_size, int n_heads, int head_dim,
                                              int kv_dim, int n_layers, int layer_idx)
{
    int batch_idx = blockIdx.x;
    int head_idx = blockIdx.y;
    ...
    int seq_len = positions[batch_idx] + 1;
    ...
    int physical_block = block_table[batch_idx * MAX_PAGES_PER_SEQ + page_idx];
    if (physical_block < 0)
    ...
    const __hip_bfloat16 *k_head = paged_key_cache + 
        1LL*physical_block * n_layers * PAGE_SIZE * kv_dim +
        1LL*layer_idx * PAGE_SIZE * kv_dim +
...
__global__ void paged_attention_weighted_sum_kernel(float *output, const float *att,
                                                    const __hip_bfloat16 *paged_value_cache,
                                                    const int *block_table, const int *positions,
                                                    int batch_size, int n_heads, int head_dim,
                                                    int kv_dim, int n_layers, int layer_idx)
{
    ...
    for (int token_idx = 0; token_idx < seq_len; token_idx++)
    {
        ...
        int physical_block = block_table[batch_idx * MAX_PAGES_PER_SEQ + page_idx];
        if (physical_block < 0)
            continue; // Page not allocated
        
        // Calculate value pointer in paged cache
        const __hip_bfloat16 *v_head = paged_value_cache + 
            1LL*physical_block * n_layers * PAGE_SIZE * kv_dim +
            1LL*layer_idx * PAGE_SIZE * kv_dim +
            1LL*page_offset * kv_dim +
            1LL*kv_head * head_dim;
...
__global__ void paged_update_kv_cache_kernel(__hip_bfloat16 *paged_key_cache, 
                                             __hip_bfloat16 *paged_value_cache,
                                             const float *k, const float *v,
                                             const int *block_table, const int *positions,
                                             int batch_size, int kv_dim, int n_layers, int layer_idx)
{
    int batch_idx = blockIdx.x;
    ...
    // Update key cache
    size_t k_cache_idx = 1LL*physical_block * n_layers * PAGE_SIZE * kv_dim +
                         1LL*layer_idx * PAGE_SIZE * kv_dim +
                         1LL*page_offset * kv_dim + dim_idx;
    paged_key_cache[k_cache_idx] = __float2bfloat16(k[1LL*batch_idx * kv_dim + dim_idx]);

    // Update value cache
    size_t v_cache_idx = 1LL*physical_block * n_layers * PAGE_SIZE * kv_dim +
                         1LL*layer_idx * PAGE_SIZE * kv_dim +
                         1LL*page_offset * kv_dim + dim_idx;
    paged_value_cache[v_cache_idx] = __float2bfloat16(v[1LL*batch_idx * kv_dim + dim_idx]);
}
