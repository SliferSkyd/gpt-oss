...
// ==== NEW: SwiGLU epilogue that reads BF16 input and writes BF16 ====
__global__ void swiglu_epilogue_kernel_bf16_mlp_in_bf16(
    const __hip_bfloat16 *__restrict__ mlp1_out, // [sum_tokens, 2*D]  <<< now bf16
    __hip_bfloat16 *__restrict__ gate_up_bf16, // [sum_tokens, D]    <<< bf16 out
    int D,
    int sum_tokens,
    ...
    float alpha_silu /* = 1.702f */)
{
    ...
    // Load BF16 activations and convert to FP32
    float gx = __bfloat162float(mlp1_out[2 * idx]);
    float uy = __bfloat162float(mlp1_out[2 * idx + 1]);
    ...
    gate_up_bf16[idx] = __float2bfloat16(fmaf(gx, uy, gx));
}
...
void grouped_mlp1_mxfp4_kernel_unified_A_bf16_out_bf16(
...
template <
    ...
    int PAD_K_MC>
__global__ __launch_bounds__(64 * ((WAVES_M / TW_M) * (WAVES_N / TW_N)), 2)
void grouped_mlp2_mxfp4_bias_kernel_unified_A_bf16_out_bf16(
    __hip_bfloat16 *__restrict__ C,              // [sum_tokens, N] <<< bf16 output
    const __hip_bfloat16 *__restrict__ A_bf16,   // [sum_tokens, K]
    const uint8_t *__restrict__ W_packed_layer,  // [E, ceil(N*K/2)]
    const float   *__restrict__ S_scales_f32_layer,// [E, ceil(N*K/32)]
...
template <
    ...
    int TW_M = 1, int TW_N = 2,
    int PAD_K = 0>
__global__ __launch_bounds__(64 * ((WAVES_M * WAVES_N <= 16) ? (WAVES_M * WAVES_N) : 16), 2) void grouped_mlp1_bf16_bias_kernel_tiled_optimized_outbf16(
    ...
    int E, int K, int N, int cur_tiles, hipStream_t s = nullptr)
{
    ...
    hipLaunchKernelGGL(
        (grouped_mlp1_bf16_bias_kernel_tiled_optimized_outbf16<
            WM, WN, WK, WAVES_M, WAVES_N, WAVES_K, TW_M, TW_N, PAD_K>),
        grid, block, shmem_bytes, s,
        C, A, W2, b2,
        expert_offsets, expert_counts,
