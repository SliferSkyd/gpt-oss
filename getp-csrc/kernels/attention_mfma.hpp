...
#ifndef SOFTMAX_PHI
#define SOFTMAX_PHI 12.0f
#endif

// Active-warps + indexed (for bucketing)
// - Uses nwarps_act = min(nwarps_total, n_steps)
// - Merges only active warps (neutral values for inactive)
// - 'index_map' remaps grid.y -> original batch row, so we can bucket on host
__global__ __launch_bounds__(256, 4)
void flashdecoding_fused_fastmerge_nostage_1warp8q_bf16q_bf16out_indexed(
    __hip_bfloat16 *__restrict__ output, const __hip_bfloat16 *__restrict__ q,
    const __hip_bfloat16 *__restrict__ key_cache,
    const __hip_bfloat16 *__restrict__ value_cache,
    const __hip_bfloat16 *__restrict__ sinks, const __hip_bfloat16 * /*mask*/,
    ...
    int L, int layer_idx, bool use_sw,
    size_t batch_kv_stride, size_t layer_kv_offset,
    const int* __restrict__ index_map)   // NEW
{
  const int kv_h = blockIdx.x;
  const int b    = index_map ? index_map[blockIdx.y] : blockIdx.y;
  ...
  const int nwarps_total = blockDim.x >> 6;

  // 8 sub-heads per warp (GQA=8). Each sub-head uses lanes with same 'li' across hloc.
  const int hloc = lane >> 3;         // 0..7 sub-head inside warp
  const int li   = lane & 7;          // 0..7 lane in sub-head
  const int gqa  = 8;
  const int h    = kv_h * gqa + hloc;
  if (h >= H) return;
  ...
  // Only assign real work to active warps
  const int nwarps_act = n_steps < nwarps_total ? n_steps : nwarps_total;

  int t_begin = 0, t_end = 0;
  if (wid < nwarps_act) {
    t_begin = (n_steps * wid) / nwarps_act;
    t_end   = (n_steps * (wid + 1)) / nwarps_act;
  }

  const int  kv_dim  = D * KVH;                         // == 64*KVH
  const size_t headD = (size_t)kv_h * D;
  const __hip_bfloat16* __restrict__ K0 =
      key_cache  + (size_t)b * batch_kv_stride + layer_kv_offset + headD;
  const __hip_bfloat16* __restrict__ V0 =
      value_cache+ (size_t)b * batch_kv_stride + layer_kv_offset + headD;
  ...
  float qseg[8];
  {
    ...
    const __hip_bfloat16* __restrict__ qh =
        q + (size_t)b * H * D + (size_t)h * D;
#pragma unroll
    for (int s=0; s<8; ++s) qseg[s] = __bfloat162float(qh[li + 8*s]) * inv;
  }
  ...
  for (int tloc = t_begin; tloc < t_end; ++tloc) {
    const int t_abs = t_start + tloc;
    const int tw = do_sw ? (t_abs & (SW_WINDOW - 1)) : t_abs;  // &127 when SW=128

    // Base offset for this (t, kv_h, li)
    const size_t base_off = (size_t)tw * (size_t)kv_dim + (size_t)li;
    const __hip_bfloat16* __restrict__ Kp = K0 + base_off;
    const __hip_bfloat16* __restrict__ Vp = V0 + base_off;

    // Dot (coalesced), then subgroup-8 reduce
    float part = 0.f;
#pragma unroll
    for (int s=0; s<8; ++s) { const float k = __bfloat162float(*Kp); Kp += 8; part = fmaf(qseg[s], k, part); }
    part += __shfl_xor(part, 4, 8);
    part += __shfl_xor(part, 2, 8);
    part += __shfl_xor(part, 1, 8);

    // Streaming softmax update (leader per sub-head)
    float alpha=1.f, w=0.f;
    if (li == 0) {
      const float s = part;
      const float mNew = fmaxf(m, s);
    ...
    // Numerator update (coalesced)
#pragma unroll
    for (int s=0; s<8; ++s) { const float v = __bfloat162float(*Vp); Vp += 8; out8[s] = fmaf(w, v, out8[s] * alpha); }
  }

  // ---- LDS fast-merge (write neutral if inactive) ----
  extern __shared__ float redbuf[];
  float* red_m   = redbuf;                                   // [nwarps_total][8]
  float* red_l   = red_m   + nwarps_total * 8;               // [nwarps_total][8]
  float* red_out = red_l   + nwarps_total * 8;               // [nwarps_total][64][8]

  const bool active = (wid < nwarps_act);
  if (li == 0) {
    red_m[wid * 8 + hloc] = active ? m : -INFINITY;
    red_l[wid * 8 + hloc] = active ? l : 0.f;
  }
#pragma unroll
  for (int s=0; s<8; ++s) {
    red_out[((wid * 64 + lane) * 8 + s)] = active ? out8[s] : 0.f;
  }
  __syncthreads();

  // ---- leader merge ONLY active warps ----
  if (wid == 0) {
    const int my_h  = hloc;
    const int my_li = li;
    ...
    float m_max = -INFINITY;
#pragma unroll
    for (int widx=0; widx<nwarps_act; ++widx) m_max = fmaxf(m_max, red_m[widx*8 + my_h]);

    float l_sum = 0.f, num8[8] = {0,0,0,0,0,0,0,0};
#pragma unroll
    for (int widx=0; widx<nwarps_act; ++widx) {
      const float mw = red_m[widx*8 + my_h];
      const float scale = (mw == -INFINITY) ? 0.f : __expf(mw - m_max);
      l_sum = fmaf(red_l[widx*8 + my_h], scale, l_sum);
      const int base_idx = ((widx * 64 + lane) * 8);
#pragma unroll
      for (int s=0; s<8; ++s) num8[s] = fmaf(red_out[base_idx + s], scale, num8[s]);
    }

    // Merge sink (denominator only) and write
    float alpha_sink=1.f, l_final=l_sum;
    if (my_li==0) {
      const float ss = __bfloat162float(sinks[kv_h * gqa + my_h]);
      const float m_new = fmaxf(m_max, ss);
      const float a2 = __expf(m_max - m_new);
      const float e  = __expf(ss - m_new);
      l_final   = l_sum * a2 + e;
      alpha_sink= a2;
    }
