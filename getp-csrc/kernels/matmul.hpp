...
template <
    ...
    // Fused epilogue?
    bool FUSED>
__global__ __launch_bounds__(64 * ((WAVES_M / TW_M) * (WAVES_N / TW_N)), 2) void mfma_bf16_kernel(
...
#pragma unroll
    ...
    // Preload kBase=0
    if (alignedA)
        copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, true>(sA0_u32, A, m0, M, K, 0, linearT, threadsPerBlock);
    else
        copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, false>(sA0_u32, A, m0, M, K, 0, linearT, threadsPerBlock);

    if (use128bW)
        copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, true>(sB0_u32, Wbf16, n0, N, K, 0, linearT, threadsPerBlock);
    else
        copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, false>(sB0_u32, Wbf16, n0, N, K, 0, linearT, threadsPerBlock);
...
#pragma unroll 1
    for (int k0 = 0; k0 < Kmain; k0 += BLOCK_K)
    {
        ...
        {
            if (alignedA)
                copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, true>(nextA32, A, m0, M, K, kNext, linearT, threadsPerBlock);
            else
                copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, false>(nextA32, A, m0, M, K, kNext, linearT, threadsPerBlock);

            if (use128bW)
                copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, true>(nextB32, Wbf16, n0, N, K, kNext, linearT, threadsPerBlock);
            else
                copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, false>(nextB32, Wbf16, n0, N, K, kNext, linearT, threadsPerBlock);
        }
...
#pragma unroll
    ...
    {
        if (alignedA)
            copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, true>(nextA32, A, m0, M, K, Kmain, linearT, threadsPerBlock);
        else
            copy_A_tile_vec<BLOCK_M, BLOCK_K, ldA, false>(nextA32, A, m0, M, K, Kmain, linearT, threadsPerBlock);

        if (use128bW)
            copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, true>(nextB32, Wbf16, n0, N, K, Kmain, linearT, threadsPerBlock);
        else
            copy_B_tile_vec<BLOCK_N, BLOCK_K, ldB, false>(nextB32, Wbf16, n0, N, K, Kmain, linearT, threadsPerBlock);
...
#pragma unroll
        for (int tn = 0; tn < TW_N; ++tn)
        {
            ...
            {
                if (interior)
                    store_and_fuse_tile<WM, WN, true>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt, lane);
                else
                    store_and_fuse_tile<WM, WN, false>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt, lane);
            }
            else
            {
                if (interior)
                    store_c_tile<WM, WN, true>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt, lane);
                else
                    store_c_tile<WM, WN, false>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt, lane);
            }
        }
    }
}
...
template <
    ...
    // Fused epilogue?
    bool FUSED>
__global__ __launch_bounds__(64 * ((WAVES_M / TW_M) * (WAVES_N / TW_N)), 2) void mfma_bf16_kernel_vec128_singlebuf(
    ...
    hipStream_t stream = nullptr)
{
    ...
    {
        if (!bias)
            fprintf(stderr, "matmul_vec128_singlebuf<FUSED>: bias is null\n");
    }
#endif
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
        v[i] = sB[col * ldB + (kOff + k_group * 4 + i)];
    return v;
}
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
        v[i] = sA[row * ldA + (kOff + k_group * 4 + i)];
    return v;
}
...
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {                               // 4 rows per half (we have 2 halves)
...
template <
    ...
    int PAD_K_MC,
    bool FUSED>
__global__ __launch_bounds__(32 * (2 * ((WAVES_M / TW_M) * (WAVES_N / TW_N))), 2) void mfma_bf16_kernel32_vec128_singlebuf(
    float *__restrict__ C,
    const float *__restrict__ A,
...
#pragma unroll
        for (int tn = 0; tn < TW_N; ++tn)
        {
            ...
            {
                if (interior)
                    store_and_fuse_tile32<WM, WN, true>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt);
                else
                    store_and_fuse_tile32<WM, WN, false>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt);
            }
            else
            {
                if (interior)
                    store_c_tile32<WM, WN, true>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt);
                else
                    store_c_tile32<WM, WN, false>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt);
            }
        }
    }
}
    ...
    hipStream_t stream = nullptr)
{
    ...
    {
        if (!bias)
            fprintf(stderr, "matmul32x32x8_vec128_singlebuf<FUSED>: bias is null\n");
    }
#endif
...
template <
    ...
    // Fused epilogue?
    bool FUSED>
__global__ __launch_bounds__(64 * ((WAVES_M / TW_M) * (WAVES_N / TW_N)), 2) void mfma_bf16_kernel_vec128_singlebuf_Abf16(
...
#pragma unroll
        for (int tn = 0; tn < TW_N; ++tn)
        {
            ...
            {
                if (interior)
                    store_and_fuse_tile_bf16<WM, WN, true>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt, lane);
                else
                    store_and_fuse_tile_bf16<WM, WN, false>(acc[tm][tn], C, bias, M, N, m0, n0, wmt, wnt, lane);
            }
            else
            {
                if (interior)
                    store_c_tile_bf16<WM, WN, true>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt, lane);
                else
                    store_c_tile_bf16<WM, WN, false>(acc[tm][tn], C, M, N, m0, n0, wmt, wnt, lane);
            }
        }
    }
}
    ...
    hipStream_t stream = nullptr)
{
    ...
    {
        if (!bias)
            fprintf(stderr, "matmul_vec128_singlebuf_Abf16<FUSED>: bias is null\n");
    }
#endif
...
template <
    ...
    int PAD_K_MC,
    bool FUSED>
__global__ __launch_bounds__(32 * (2 * ((WAVES_M / TW_M) * (WAVES_N / TW_N))), 2) void mfma_bf16_kernel32_vec128_singlebuf_Abf16(
    __hip_bfloat16 *__restrict__ C,
    const __hip_bfloat16 *__restrict__ A, // [M,K] bf16  <-- changed
    const __hip_bfloat16 *__restrict__ Wbf16,
    const __hip_bfloat16 *__restrict__ bias,
    ...
    hipStream_t stream = nullptr)
{
    ...
    {
        if (!bias)
            fprintf(stderr, "matmul32x32x8_vec128_singlebuf_Abf16<FUSED>: bias is null\n");
    }
#endif
...
template <typename T>
void getp_matmul(float *xout, const float *x, const T *w, const T *b, int n, int d,
                 int batch_size)
{
...
// -------------------------------------------------------------------------------------
// Small-M optimized launcher (M in {1,2,3,4}, tuned for N=1440, K=2880)
// Uses the ping–pong (double-buffer) kernel `mfma_bf16_kernel` via the `matmul<>` wrapper.
// Configuration rationale:
//  - WAVES_M=1 keeps BM=16 so we only overcompute up to 16 rows (cheap) while maximizing N parallelism
//  - WAVES_N=16 -> BLOCK_N = 16*16 = 256, grid.x ≈ 1440/256 = 6 CTAs
//  - WAVES_K=4  -> BLOCK_K = 64 (good balance for K=2880 -> 45 steps)
//  - PAD_K_MC=16 to avoid LDS bank conflicts
// -------------------------------------------------------------------------------------
inline void matmul_smallM_opt(
    ...
    hipStream_t stream = nullptr)
{
    getp_matmul<__hip_bfloat16>(C, A, Wbf16, nullptr, K, N, M);
}